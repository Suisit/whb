  ...
  <div style="padding: 50px 0 0 70px; width: 1100px">
    ...
    >
      ...
      >
        <a-input
          placeholder="请输入标题"
          v-model="dynamicValidateForm.work.title"
        ></a-input>
      </a-form-model-item>
...
export default {
  ...
  methods: {
    async fetch() {
      const res = await this.$http.get(`works/${this.workId}`,{
        params:{
          query:{
            populate: [
              { path: "selectQuestions"},
              { path: "judgmentQuestions" }
            ],
          }
        }
      });
      ...
      this.dynamicValidateForm.work.title = res.data.title;
      this.dynamicValidateForm.work.totalScore = res.data.totalScore;
      this.dynamicValidateForm.selectQuestions = res.data.selectQuestions;
      this.dynamicValidateForm.judgmentQuestions = res.data.judgmentQuestions;
    },
    submitForm(formName) {
      this.dynamicValidateForm.work.totalScore = this.addtotalScore;
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          const { data } = await this.$http.put(
            `works/${this.workId}`,
            this.dynamicValidateForm.work
          );
          for (let i of this.dynamicValidateForm.selectQuestions) {
            if (i.object) {
              await this.$http.put(`select-questions/${i._id}`, i);
            } else {
              i.object = data._id;
          ...
          for (let i of this.dynamicValidateForm.judgmentQuestions) {
            if (i.object) {
              await this.$http.put(`judgment-questions/${i._id}`, i);
            } else {
              i.object = data._id;
              await this.$http.post(
                "judgment-questions",
          ...
          console.log(this.dynamicValidateForm.work);
          return false;
        }
      });
    },
