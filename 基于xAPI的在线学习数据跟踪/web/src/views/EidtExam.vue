...
export default {
  ...
  methods: {
    async fetch() {
      const res = await this.$http.get(`exam/${this.examId}`,{
        params:{
          query:{
            populate: [
      ...
      this.dynamicValidateForm.exam.title = res.data.title;
      this.dynamicValidateForm.exam.totalScore = res.data.totalScore;
      ...
      this.dynamicValidateForm.exam.time=res.data.time;
    },
    submitForm(formName) {
